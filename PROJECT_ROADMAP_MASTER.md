...
- **PAT-free (20/08/2026):** `collect.yml` migrado para GitHub App — o passo "Mint GitHub App token" (`actions/create-github-app-token@v1`) mints um installation token de 1h por run e a rotação do secret usa `steps.app_token.outputs.token` com fallback `GH_PAT` (zero downtime). **Concluído e validado em produção:** App criada (ID 4654885), 2 secrets gravados (`APP_ID`, `APP_PRIVATE_KEY`), `GH_PAT` removido e run PAT-free verde com rotação viva de `SF_REFRESH_TOKEN` (20/08/2026 01:19:45Z). Critério de aceite do kit (docs/VALIDACAO §5) satisfeito. Detalhes de implementação: secret names sem prefixo `GITHUB_` (HTTP 422), `if:` do mint via job env (secrets context não é permitido em step `if`), e sem `installation-id` (a ação resolve a instalação via `owner`).
- **Kit reutilizável publicado:** https://github.com/brunotrolo/Salesforce_MCPauthentication (commit `1dc6c55`, v1.1.0) — repositório canônico e única fonte do kit (`src/sf_mcp_auth/` instalável, `scripts/get_sf_mcp_tokens.py`, `pipeline/run_with_rotation.py`, `workflow/collect.yml`, `docs/TROUBLESHOOTING.md` + `docs/VALIDACAO.md`). Homologado em produção: 39 tests passando, runs verdes com rotação viva do secret PAT-free (20/08/2026). Réplica verificada em venv limpa. Sem secrets no código (validado). v1.1.0: módulo `github_app_token.py` (mint de token de instalação, PAT-free) + 39 tests.
