...
- **PAT-free (19/08/2026):** `collect.yml` migrado para GitHub App — o passo "Mint GitHub App token" (`actions/create-github-app-token@v1`) mints um installation token de 1h por run e a rotação do secret usa `steps.app_token.outputs.token` com fallback `GH_PAT` (zero downtime). Setup único do usuário pendente: criar a App + 3 secrets (`GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_INSTALLATION_ID`) e remover `GH_PAT` — validar pelo critério de aceite do kit (docs/VALIDACAO §5).
- **Kit reutilizável publicado:** https://github.com/brunotrolo/Salesforce_MCPauthentication (commit `743e5e9`, v1.1.0) — repositório canônico e única fonte do kit (`src/sf_mcp_auth/` instalável, `scripts/get_sf_mcp_tokens.py`, `pipeline/run_with_rotation.py`, `workflow/collect.yml`, `docs/TROUBLESHOOTING.md` + `docs/VALIDACAO.md`). Homologado em produção: 35 tests passando, 2 runs verdes consecutivas com rotação viva do secret (19/08/2026). Réplica verificada em venv limpa (`pip install .` + pytest = 35 passed). Sem secrets no código (validado). v1.1.0: módulo `github_app_token.py` (mint de token de instalação, PAT-free) + 39 tests.
